...
<p>
	<a href="/admin/information_db_providers/" class="btn btn-dark"><< Назад</a>
</p>
...
$sql = "SELECT * FROM `db_providers` WHERE `id`='{$faq_id}'";
...
<div class="container">
	<form method="POST">
		...
		<div class="form-group">
			<label for="inputQuestion" class="font-weight-bold">Название Поставщика</label>
			<input type="text" name="question" class="form-control" id="inputQuestion" placeholder="Введите название" value="<?=$faq['question']?>" required>
		</div>
		<div class="form-group">
			<label for="inputAnswer" class="font-weight-bold">Информация о поставщике</label>
			<textarea name="answer" rows="7" id="inputAnswer" class="form-control faq-description summernote" placeholder="Введите информацию..."><?=$faq['answer']?></textarea>
		</div>
		<div class="form-group">
			<label for="inputWhom" class="font-weight-bold">Категория</label>
			<select name="whom" id="inputWhom" class="form-control" required>
				<option value="1" <?=(($faq['whom']==1)?' selected':'')?>>Товары для детей</option>
...
// Переменная хранит число сообщений выводимых на станице
		...
		$sql = "SELECT COUNT(1) as count FROM `db_providers`";
		if (!empty($_SESSION['faq_filter']))
			$sql = "SELECT COUNT(1) as count FROM `db_providers` WHERE `whom`='{$_SESSION['faq_filter']}'";
		$query = mysqli_query($db, $sql) or die(mysqli_error($db));
					...
					<div class="row">
						<div class="col-sm-8">
							<select name="whom" id="inputWhom" class="form-control">
								...
								<option value="1" <?=(($_SESSION['faq_filter']==1)?' selected':'')?>>Товары для детей</option>
...
<?
		...
		$sql = "SELECT * FROM `db_providers` WHERE `lang`='ru' ORDER BY `sort` ASC, `created` DESC LIMIT $start, $num";
		if (!empty($_SESSION['faq_filter']))
			$sql = "SELECT * FROM `db_providers` WHERE `whom`='{$_SESSION['faq_filter']}' AND `lang`='ru' ORDER BY `sort` ASC, `created` DESC LIMIT $start, $num";
		...
		if (mysqli_num_rows($query) > 0) {
				...
				while ($faq = mysqli_fetch_assoc($query)) {
					...
					switch ($faq['whom']) {
						case 1:
							$faq_whom_word = 'Товары для детей';
							break;
						/*case 2:
...
?>
						...
						<tr>
							...
							<td>
								<a href="/admin/information_db_providers/?question=<?=$faq['id']?>"><?=$faq['question']?></a>
							</td>
							<td><?=$faq_whom_word?></td>
							<td><?=$faq['sort']?></td>
							<td>
								<form method="POST" class="float-left">
									...
									<input type="hidden" name="id" value="<?=$faq['id']?>">
									<button type="submit" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="top" title="Удалить" onclick="return confirm('Вы действительно хотите удалить этого поставщика?')">
...
<?
				...
				echo '<tr><td colspan="5" class="text-center pt-3 pb-3">Еще нет поставщиков</td></tr>';
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="POST">
					...
					<div class="form-group">
						<label for="inputQuestion" class="font-weight-bold">Название Поставщика</label>
						<input type="text" name="question" class="form-control" id="inputQuestion" placeholder="Введите название" required>
					</div>
					<div class="form-group">
						<label for="inputAnswer" class="font-weight-bold">Информация о поставщике</label>
						<textarea name="answer" rows="7" id="inputAnswer" class="form-control faq-description summernote" placeholder="Введите информацию..."></textarea>
					</div>
					<div class="form-group">
						<label for="inputWhom" class="font-weight-bold">Категория</label>
						<select name="whom" id="inputWhom" class="form-control" required>
							<option value="1" selected>Товары для детей</option>
