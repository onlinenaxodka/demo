...
// Переменная хранит число сообщений выводимых на станице
		...
		$sql = "SELECT COUNT(1) as count FROM `transactions` WHERE `type`=0";
	...
	<table class="table table-sm table-hover" style="font-size:14px">
		<thead class="thead-default">
			<tr>
				...
				<th>ЭПС</th>
				<th style="min-width:236px">Статус</th>
				<th>Создана</th>
			</tr>
		</thead>
		<tbody>
		...
		$sql = "SELECT * FROM `transactions` WHERE `type`=0 ORDER BY FIELD(`status`,1,2,0) ASC, `created` DESC LIMIT $start, $num";
		$query = mysqli_query($db, $sql) or die(mysqli_error());

		if (mysqli_num_rows($query) == 0) {

			echo '<tr><td colspan="9" class="text-sm-center">Пополнений баланса еще не было</td></tr>';
		...
		while ($transactions = mysqli_fetch_assoc($query)) {
			...
			if ($transactions['status'] == 0) {
				...
				if ($transactions['updated'] == $transactions['created']) {
					...
					$color_text = '#757575';
					$transactions['status'] = 'Создана';

				} else {

					$color_text = '#dfb81c';
					$transactions['status'] = 'Ожидает подтверждения<form action="/admin/add_funds/" method="POST" style="display:inline-block"><input type="hidden" name="status_confirm" value="' . $transactions['id'] . '"><input class="form-control" type="number" step="0.01" placeholder="Введите сумму" name="cash" value="' . $transactions['add_funds'] . '" style="width:87.5%;margin:5px 0"><input type="submit" class="btn btn-success btn-sm" value="Подтвердить"></form> <form action="/admin/add_funds/" method="POST" style="display:inline-block"><input type="hidden" name="status_cancel" value="' . $transactions['id'] . '"><input type="submit" class="btn btn-danger btn-sm" value="Отменить" style="margin:2px 0 0 -120px"></form>';
			...
			<tr>
				...
				<td>' . $transactions['add_funds'] . ' грн</td>
				<td>' . $transactions['change'] . ' грн</td>
