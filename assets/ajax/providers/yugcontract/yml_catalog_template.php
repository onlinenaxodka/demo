...
foreach ($xml->shop->categories->category as $category) {
	...
	$sql = "SELECT `id` FROM `catalog` WHERE `user_id`=4479 AND `group_id`='{$category_id}' LIMIT 1";
	...
	if ($count_catalog == 0) {
		...
		$sql = "INSERT INTO `catalog` SET `level_id`=6657,
	    									`user_id`=4479,
...
$sql_ce = "SELECT `id`, `group_id`, `group_parent_id` FROM `catalog` WHERE `user_id`=4479";
...
if (mysqli_num_rows($query_ce) > 0) {
	...
	while ($catalog_ce = mysqli_fetch_assoc($query_ce)) {
		...
		if ($catalog_ce['group_parent_id'] > 0) {
			...
			$sql_ce_up = "SELECT `id` FROM `catalog` WHERE `user_id`=4479 AND `group_id`='{$catalog_group_parent_id}' LIMIT 1";
...
foreach ($xml->shop->offers->offer as $offer) {
	...
	$sql = "SELECT * FROM `goods` WHERE `vendor_id`='{$goods_vendor_id}' AND `user_id`=4479 LIMIT 1";
	$query = mysqli_query($db, $sql) or die(mysqli_error($db));
	...
	if ($count_goods > 0) {
		...
		if (!empty($photo)) {
			...
			$sql = "UPDATE `goods` SET `photo`='{$photo_str}' WHERE `id`='{$goods_id}' AND `user_id`=4479";
			$query = mysqli_query($db, $sql) or die(mysqli_error($db));
		...
		$sql = "UPDATE `goods` SET `availability`='{$goods_availability}',
									...
									`status`='{$status}',
									`status_import`=1,
									`updated`='{$current_date}' WHERE `id`='{$goods_id}' AND `user_id`=4479";
		$query = mysqli_query($db, $sql) or die(mysqli_error($db));

	} else {

		$sql = "SELECT `linkname` FROM `catalog` WHERE `user_id`=4479 AND `group_id`='{$category_id}' LIMIT 1";
		$query = mysqli_query($db, $sql) or die(mysqli_error($db));
		$catalog_category = mysqli_fetch_assoc($query);
		$catalog_category_linkname = $catalog_category['linkname'];

		$sql = "INSERT INTO `goods` SET `user_id`=4479,
...
$sql = "UPDATE `goods` SET `availability`=0, `status`=0, `updated`='{$current_date}' WHERE `status_import`=0 AND `user_id`=4479";
$query = mysqli_query($db, $sql) or die(mysqli_error($db));

$sql = "UPDATE `goods` SET `status_import`=0, `updated`='{$current_date}' WHERE `user_id`=4479";
$query = mysqli_query($db, $sql) or die(mysqli_error($db));
