...
foreach ($soap_categories_xml->Body->GetCategoriesResponse->Categories->category as $category) {

	$category_name = test_request($category['name']);
	$category_name = str_replace("'", "\'", $category_name);

	$category_id = test_request($category['id']);
	$category_id = intval($category_id);

	$category_parent_id = test_request($category['parent_id']);
	$category_parent_id = intval($category_parent_id);

	$sql = "SELECT `id` FROM `catalog` WHERE `user_id`=6770 AND `group_id`='{$category_id}' LIMIT 1";
	...
	if ($count_catalog == 0) {
		...
		$sql = "INSERT INTO `catalog` SET `level_id`=7068,
	    									`user_id`=6770,
...
$sql_ce = "SELECT `id`, `group_id`, `group_parent_id` FROM `catalog` WHERE `user_id`=6770";
...
if (mysqli_num_rows($query_ce) > 0) {
	...
	while ($catalog_ce = mysqli_fetch_assoc($query_ce)) {
		...
		if ($catalog_ce['group_parent_id'] > 0) {
			...
			$sql_ce_up = "SELECT `id` FROM `catalog` WHERE `user_id`=6770 AND `group_id`='{$catalog_group_parent_id}' LIMIT 1";
...
foreach ($soap_offers_xml->Body->GetContentGoodsResponse->yml_catalog->offers->offer as $offer) {

	$goods_vendor_id = test_request($offer['id']);

	$sql = "SELECT * FROM `goods` WHERE `vendor_id`='{$goods_vendor_id}' AND `user_id`=6770 LIMIT 1";
	$query = mysqli_query($db, $sql) or die(mysqli_error($db));
	$goods = mysqli_fetch_assoc($query);
	$count_goods = mysqli_num_rows($query);
			
	$goods_name = test_request($offer->name_ru);
	...
	if (count($offer->param) > 0) {
		...
		foreach ($offer->param as $param) {
			...
			if ($param['lang'] == 'ru') {
				...
				if ($param['name'] == 'Артикул') {
					...
					$goods_vendor_code = test_request($param);
		...
		$goods_template = "{\"uk\":{\"Производитель\":\"".$goods_vendor."\",\"ru\":{\"Производитель\":\"".$goods_vendor."\"}}";

	}

	$description_uk = test_request($offer->description_ru);
	$description_uk = str_replace("'", "\'", $description_uk);

	$description_ru = test_request($offer->description_ru);
	...
	for ($i=0; $i < count($photo_tmp); $i++) { 
		$photo['img'.$i] = $photo_tmp[$i];
	}
	...
	$goods_currency = 1;
	$goods_currency_top_kurs = 0;

	//$goods_availability = intval(test_request($offer->stock_quantity));
	...
	if ($count_goods > 0) {
		...
		if (!empty($photo)) {
			...
			$sql = "UPDATE `goods` SET `photo`='{$photo_str}' WHERE `id`='{$goods_id}' AND `user_id`=6770";
			$query = mysqli_query($db, $sql) or die(mysqli_error($db));
		...
		/*$sql = "UPDATE `goods` SET `availability`='{$goods_availability}',
									...
									`status`='{$status}',
									`status_import`=1,
									`updated`='{$current_date}' WHERE `id`='{$goods_id}' AND `user_id`=6770";
		$query = mysqli_query($db, $sql) or die(mysqli_error($db));*/

	} else {

		$sql = "SELECT `linkname` FROM `catalog` WHERE `user_id`=6770 AND `group_id`='{$category_id}' LIMIT 1";
		$query = mysqli_query($db, $sql) or die(mysqli_error($db));
		$catalog_category = mysqli_fetch_assoc($query);
		$catalog_category_linkname = $catalog_category['linkname'];

		$sql = "INSERT INTO `goods` SET `user_id`=6770,
...
$sql = "UPDATE `goods` SET `availability`=0, `status`=0, `updated`='{$current_date}' WHERE `status_import`=0 AND `user_id`=6770";
$query = mysqli_query($db, $sql) or die(mysqli_error($db));

$sql = "UPDATE `goods` SET `status_import`=0, `updated`='{$current_date}' WHERE `user_id`=6770";
$query = mysqli_query($db, $sql) or die(mysqli_error($db));
