...
$file_url = 'https://carassius.com.ua/yandex_market.xml?hash_tag=5d2a4f47c4257804554e930c5985856a&sales_notes=&product_ids=&label_ids=&exclude_fields=&html_description=1&yandex_cpa=&process_presence_sure=&group_ids=';
$file_local = __DIR__ . '/../../../data/files/import/carassius.xml';
...
if (filter_var($xml_file, FILTER_VALIDATE_URL) !== false) {
	...
	foreach ($xml->shop->categories->category as $category) {
		...
		$sql = "SELECT `id` FROM `catalog` WHERE `user_id`=7203 AND `group_id`='{$category_id}' LIMIT 1";
	    ...
	    if ($count_catalog == 0) {
	    	...
	    	$sql = "INSERT INTO `catalog` SET `level_id`=6862,
	    										`user_id`=7203,
	...
	$sql_ce = "SELECT `id`, `group_id`, `group_parent_id` FROM `catalog` WHERE `user_id`=7203";
	...
	if (mysqli_num_rows($query_ce) > 0) {
		...
		while ($catalog_ce = mysqli_fetch_assoc($query_ce)) {
			...
			if ($catalog_ce['group_parent_id'] > 0) {
				...
				$sql_ce_up = "SELECT `id` FROM `catalog` WHERE `user_id`=7203 AND `group_id`='{$catalog_group_parent_id}' LIMIT 1";
		...
		foreach ($xml->shop->offers->offer as $offer) {
			...
			$sql = "SELECT * FROM `goods` WHERE `vendor_id`='{$goods_vendor_id}' AND `user_id`=7203 LIMIT 1";
			$query = mysqli_query($db, $sql) or die(mysqli_error($db));
			...
			$goods_country = test_request($offer->country_of_origin);
			...
			$count_pictures = count($offer->picture);
			//$count_pictures = 0;

			if ($count_pictures > 0) {

				if ($photo_count != $count_pictures) {
			...
			if ($count_goods > 0) {
				...
				if (!empty($photo)) {
					...
					$sql = "UPDATE `goods` SET `photo`='{$photo_str}' WHERE `id`='{$goods_id}' AND `user_id`=7203";
					$query = mysqli_query($db, $sql) or die(mysqli_error($db));
				...
				$sql = "UPDATE `goods` SET `availability`='{$goods_availability}',
											...
											`status`='{$status}',
											`status_import`=1,
											`updated`='{$current_date}' WHERE `id`='{$goods_id}' AND `user_id`=7203";
				$query = mysqli_query($db, $sql) or die(mysqli_error($db));

			} else {

				$sql = "SELECT `linkname` FROM `catalog` WHERE `user_id`=7203 AND `group_id`='{$category_id}' LIMIT 1";
				$query = mysqli_query($db, $sql) or die(mysqli_error($db));
				if (mysqli_num_rows($query) > 0) {
					$catalog_category = mysqli_fetch_assoc($query);
					$catalog_category_linkname = $catalog_category['linkname'];
				} else {
					$catalog_category_linkname = 'tovari_bez_kategorii';
				}

				$sql = "INSERT INTO `goods` SET `user_id`=7203,
		...
		$sql = "UPDATE `goods` SET `availability`=0, `status`=0, `updated`='{$current_date}' WHERE `status_import`=0 AND `user_id`=7203";
		$query = mysqli_query($db, $sql) or die(mysqli_error($db));

		$sql = "UPDATE `goods` SET `status_import`=0, `updated`='{$current_date}' WHERE `user_id`=7203";
		$query = mysqli_query($db, $sql) or die(mysqli_error($db));
