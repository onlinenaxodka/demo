...
$linkname = (isset($_GET['linkname'])) ? mysqli_real_escape_string($db, $_GET['linkname']) : '';
...
$sql = "SELECT * FROM `goods` WHERE `category` IN ({$category_linkname_separated}) AND `status`=1 AND `availability`>0";
$query = mysqli_query($db, $sql) or die(mysqli_error($db));
...
if ($xml->save('yml_our/rozetka.xml')) {

	echo 'Обновление фида rozetka завершилось успешно!';

} else {

	echo 'Не удалось сохранить файл rozetka фида данных. Возможно у файла не достаточно прав доступа.';

	file_put_contents("errors/yml_our_rozetka.log", date('Y-m-d H:i:s')." - Не удалось сохранить файл rozetka.xml фида данных. Возможно у файла не достаточно прав доступа.\n", FILE_APPEND | LOCK_EX);
...
$xml->save('yml_our/rozetka.xml'); #-> сохраняем файл
