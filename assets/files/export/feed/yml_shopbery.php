...
foreach ($catalog_arr as $catalog_arr_value) {
	...
	if ($catalog_parent_id == 1) {
		...
		$category->setAttribute('img', 'https://onlinenaxodka.com/data/images/catalog/'.$catalog_img);
		...
		while ($goods = mysqli_fetch_assoc($query_goods)) {
			...
			for ($i=0; $i < $goods_photo_total; $i++) {
				...
				if (file_exists(__DIR__.'/../../../images/goods/'.$goods['photo']['img'.$i])) {
					...
					$picture->appendChild($xml->createTextNode($domen_name.'/data/images/goods/'.$goods['photo']['img'.$i]));
...
if ($xml->save(__DIR__.'/yml_shopbery.xml')) {

	echo "Оновлення фіду yml_shopbery.xml успішно завершилося!\n";

} else {

	echo "Неможливо зберегти файл yml_shopbery.xml фіда даних. Можливо у файлу недостатньо прав доступу.\n";
