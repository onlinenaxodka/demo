...
while ($catalog = mysqli_fetch_assoc($query)) {
	...
	$catalog_arr = categories($db, $catalog['id'], $catalog['level_id'], array($catalog['linkname'], $catalog['name_uk'], $catalog['name_ru'], $catalog['img']), $catalog_arr);
...
foreach ($catalog_arr as $catalog_arr_value) {
	...
	$catalog_name_uk = $catalog_arr_value['data'][1];
	$catalog_name = $catalog_arr_value['data'][2];
	$catalog_img = $catalog_arr_value['data'][3];
	...
	if ($catalog_parent_id == 1) {
		...
		$category->setAttribute('name_uk', $catalog_name_uk);
...
if (count($category_linkname_children) > 0) {
	...
	/*$sql_goods = "
	SELECT g.id, g.availability, g.vendor_code, g.name, g.category, g.currency, g.currency_top_kurs, g.price_sale, g.photo, g.parameters, g.keys, c.id AS catalog_id, gd.description FROM goods g 
	LEFT JOIN catalog c ON c.linkname=g.category 
	LEFT JOIN goods_description gd ON gd.goods_id=g.id 
	WHERE g.user_id IN ({$user_ids}) AND g.category IN ({$category_linkname_separated}) AND g.photo NOT LIKE '%{\"img0\":\"no_image.png\"}%' AND g.availability>0 AND g.status=1 AND gd.lang='ru'";*/
	$sql_goods = "
	SELECT g.id, g.availability, g.vendor_code, g.name, g.category, g.currency, g.currency_top_kurs, g.price_sale, g.photo, g.parameters, g.keys, c.id AS catalog_id, gd.description FROM goods g 
	LEFT JOIN catalog c ON c.linkname=g.category 
	LEFT JOIN goods_description gd ON gd.goods_id=g.id 
	WHERE g.user_id IN ({$user_ids}) AND g.category IN ({$category_linkname_separated}) AND g.photo NOT LIKE '%{\"img0\":\"no_image.png\"}%' AND g.status=1 AND gd.lang='ru'";
	...
	if (mysqli_num_rows($query_goods) > 0) {
		...
		while ($goods = mysqli_fetch_assoc($query_goods)) {
			...
			//if ($goods['parameters']['uk']['Тип'] == 'зерновий сухий маточний') $price_sale = $price_sale * 1.1;
			//if ($goods['parameters']['uk']['Тип'] == 'зерновий живий маточний') $price_sale = $price_sale * 1.3;
			...
			if (!empty($goods['parameters']['ru']['Производитель']))
				$goods_vendor = $goods['parameters']['ru']['Производитель'];
			if (!empty($goods['parameters']['ru']['Бренд']))
				$goods_vendor = $goods['parameters']['ru']['Бренд'];
			...
			/*$sql_goods_description = "SELECT `description`, `lang` FROM `goods_description` WHERE `goods_id`='{$goods['id']}'";
			...
			while ($goods_description = mysqli_fetch_assoc($query_goods_description)) {
				...
				if ($goods_description['lang'] == 'ru') {
					$goods_description_ru = clearStr($goods_description['description']);
				}

			}*/
			$goods_description_ru = clearStr($goods['description']);

			$description = $offer->appendChild($xml->createElement('description'));
			//$description->appendChild($xml->createCDATASection($goods_description_ru));
			$description->appendChild($xml->createTextNode($goods_description_ru));
...
if ($xml->save(__DIR__.'/yml.xml')) {

	echo "The yml.xml feed update completed successfully!\n";

} else {

	echo "Failed to save data feed yml.xml file. The file may not have sufficient permissions.\n";
