...
file_put_contents("logs/yml_rozetka.log", date('Y-m-d H:i:s')." - Start create XML docs.\n", FILE_APPEND | LOCK_EX);
...
$sql = "SELECT COUNT(1) AS count_up FROM `catalog` WHERE `id` > 1 AND `yml_rozetka`=0 AND `count_goods`<=5000";
$query = mysqli_query($db, $sql) or die(mysqli_error($db));
$catalog_count_up = mysqli_fetch_assoc($query);

if ($catalog_count_up['count_up'] == 0) {

	$sql = "UPDATE `catalog` SET `yml_rozetka`=0, `updated`='{$current_date}' WHERE `yml_rozetka`=1";
	$query = mysqli_query($db, $sql) or die(mysqli_error($db));

}

$sql = "SELECT `id`, `linkname`, `name_ru` FROM `catalog` WHERE `id` > 1 AND `yml_rozetka`=0 AND `count_goods`<=5000";
$query = mysqli_query($db, $sql) or die(mysqli_error($db));
...
if (mysqli_num_rows($query_goods) > 0) {
	...
	while ($goods = mysqli_fetch_assoc($query_goods)) {
		...
		if ($selected_category_id = 582) $price_sale = ceil($price_sale + $price_sale * 0.07);
		...
		$offer->setAttribute('available', 'true'); //атрибуты
			...
			$url = $offer->appendChild($xml->createElement('url'));
			//$url->appendChild($xml->createTextNode(''));
			...
			$categoryId->appendChild($xml->createTextNode($goods['catalog_id']));
			...
			for ($i=0; $i < $goods_photo_count; $i++) {
				...
				if (file_exists('../../images/goods/'.$goods['photo']['img'.$i])) {
					...
					/*list($goods_photo_width, $goods_photo_height) = getimagesize('../../images/goods/'.$goods['photo']['img'.$i]);

					if ($goods_photo_width >= 600 and $goods_photo_height >= 500) {*/
			...
			$name->appendChild($xml->createTextNode($goods['name']['ru'].' ('.$goods['id'].')'));
			...
			$description->appendChild($xml->createTextNode($goods['description']));
...
if ($xml->save('yml_rozetka/'.$linkname.'.xml')) {

	//echo 'Обновление фида '.$linkname.' завершилось успешно!';
	$sql_up = "UPDATE `catalog` SET `yml_rozetka`=1, `updated`='{$current_date}' WHERE `id`='{$selected_category_id}'";
	...
	file_put_contents("errors/yml_rozetka.log", date('Y-m-d H:i:s')." - Не удалось сохранить файл ".$linkname.".xml фида данных. Возможно у файла не достаточно прав доступа.\n", FILE_APPEND | LOCK_EX);

}

$xml->save('yml_rozetka/'.$linkname.'.xml'); #-> сохраняем файл
...
file_put_contents("logs/yml_rozetka.log", date('Y-m-d H:i:s')." - Finish create XML docs.\n", FILE_APPEND | LOCK_EX);
