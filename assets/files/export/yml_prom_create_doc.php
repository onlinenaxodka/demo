...
$sql = "SELECT COUNT(1) AS count_up FROM `catalog` WHERE `id` > 1 AND `yml_prom`=0 AND `locked`=0 AND `buffer`=0";
$query = mysqli_query($db, $sql) or die(mysqli_error($db));
...
$sql = "SELECT `id`, `linkname`, `name_ru` FROM `catalog` WHERE `id` > 1 AND `yml_prom`=0 AND `locked`=0 AND `buffer`=0";
//$sql = "SELECT `id`, `linkname`, `name_ru` FROM `catalog` WHERE `id` = 2992 AND `yml_prom`=0";
$query = mysqli_query($db, $sql) or die(mysqli_error($db));
...
$start_time_catalog = microtime(true);
...
$sql_goods = "SELECT g.id, g.availability, g.vendor_code, g.name, c.id AS catalog_id, g.currency, g.currency_top_kurs, g.price_sale, g.photo, g.parameters, gd.description, g.keys FROM goods g LEFT JOIN catalog c ON c.linkname=g.category LEFT JOIN goods_description gd ON gd.goods_id=g.id WHERE g.category IN ({$category_linkname_separated}) AND g.availability>0 AND g.status=1 AND gd.lang='ru'";
...
$finish_time_catalog = microtime(true) - $start_time_catalog;
$finish_time_catalog = number_format($finish_time_catalog, 0, '.', '');

echo $n." - ".$finish_time_catalog." sec.\n";
...
function categoriesLinkname($db, $category_id, $catalog_arr) {
	...
	if ($count_subcategories > 0) {
		...
		if (mysqli_num_rows($query) > 0) {
			...
			$sql = "SELECT COUNT(1) AS count FROM `goods` WHERE `category`='{$category['linkname']}' AND `status`=1 AND `availability` > 0";
			$query = mysqli_query($db, $sql) or die(mysqli_error($db));
			$goods_in_category = mysqli_fetch_assoc($query);

			if ($goods_in_category['count'] > 0) {
