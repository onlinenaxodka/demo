...
file_put_contents("logs/yml_provider.log", date('Y-m-d H:i:s')." - Start create XML docs.\n", FILE_APPEND | LOCK_EX);
...
$sql = "SELECT COUNT(1) AS count_up FROM `catalog` WHERE `id` > 1 AND `yml_prom`=0 AND `count_goods`<=3000";
$query = mysqli_query($db, $sql) or die(mysqli_error($db));
...
$sql = "SELECT `id`, `linkname`, `name_ru` FROM `catalog` WHERE `id` > 1 AND `yml_prom`=0 AND `count_goods`<=3000";
...
$sql_goods = "SELECT *, (SELECT `id` FROM `catalog` WHERE `linkname`=`goods`.`category`) AS catalog_id, (SELECT `description` FROM `goods_description` WHERE `goods_id`=`goods`.`id` AND `lang`='ru') AS description FROM `goods` WHERE `category` IN ({$category_linkname_separated}) AND `status`=1";
...
if ($xml->save('yml_prom/'.$linkname.'.xml')) {
	...
	file_put_contents("errors/yml_provider.log", date('Y-m-d H:i:s')." - Не удалось сохранить файл ".$linkname.".xml фида данных. Возможно у файла не достаточно прав доступа.\n", FILE_APPEND | LOCK_EX);
...
file_put_contents("logs/yml_provider.log", date('Y-m-d H:i:s')." - Finish create XML docs.\n", FILE_APPEND | LOCK_EX);
