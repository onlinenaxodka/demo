...
if ($orders_post_user_id == 7037) {
	...
	for ($i=0; $i < count($orders_post_goods); $i++) {
		...
		if ($goods_price_agent > 0) {

			$goods_prices_margin = ($goods_price_drop - $goods_price_agent) * $goods_availability;

		} else {

			$goods_prices_margin = ($goods_price_drop - $goods_price_purchase) * $goods_availability;

		}

		$our_shop += $goods_prices_margin;
...
for ($i=0; $i < count($orders_post_goods); $i++) {
	...
	$admin_o +=    $goods_prices_margin * $marketing['sasha'] * 0.01;
...
$admin_o = intval($admin_o * 100) / 100;
...
$sum_system_profit = $fond + $admin_r + $admin_z + $admin_t + $admin_d + $admin_o + $director + $supervisor + $manager + $admin_on_rate_dif;
...
if ($count_users_gtm > 0) {
	...
	$sum_system_profit = $fond + $admin_r + $admin_z + $admin_t + $admin_d + $admin_o + $director + $supervisor + $manager + $marketer + $admin_on_rate_dif;
