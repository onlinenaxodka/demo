...
<?
		...
		?>
		<h4 class="mt-5">Где я продаю?</h4>
		<?

		$sql = "SELECT * FROM `users_shops` WHERE `user_id`={$mentor['id']}";
		$query = mysqli_query($db, $sql) or die(mysqli_error($db));
...
?>
		...
		<p>
			<span><?=$n?>.</span>
			<a href="<?=$users_shops['url']?>" target="_blank" class="btn btn-info float-right"><?=$users_shops['type']?></a>
		</p>
